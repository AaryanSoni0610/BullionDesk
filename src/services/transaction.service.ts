      ...
      return new Promise<boolean>((resolve) => {
        db.withTransactionAsync(async () => {
          try {
            ...
            // Recalculate daily opening balances from the restored transaction's date onwards
            // This is crucial because restoring a transaction in the past changes all subsequent opening balances
            const transactionDate = new Date(transaction.date);
            await InventoryService.recalculateBalancesFrom(transactionDate.toISOString());
            
            resolve(true);
          } catch (error) {
            console.error('Error in restore transaction DB transaction:', error);
            resolve(false);
          }
        });
      });
